...
void output_bit( int bit )
{
    ...
    if ( output_mask == 0 )
    {
        ...
        {
            fwrite( buffer, 1, BUFFER_SIZE, compressed_file );
            current_byte = buffer;
        }
        *current_byte = 0;
    }
...
{
    fwrite( buffer, 1, (size_t)( current_byte - buffer ) + 1, compressed_file );
    current_byte = buffer;
}
...
short int input_bit( )
{
    if ( input_bits_left == 0 )
    {
        ...
        {
            input_bytes_left = fread( buffer, 1, BUFFER_SIZE, compressed_file );
            if ( input_bytes_left == 0 )
            {
                if ( past_eof )
                {
...
{
    return( ftell( compressed_file ) - input_bytes_left + 1 );
}
